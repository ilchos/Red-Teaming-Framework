...
def do_bench(init_lang='en', agent_file_name="agent_outputs.json", debug_testing=False):
    ...
    with open(os.path.abspath(f"../outputs/{agent_file_name}.json"), "r", encoding='utf-8') as fp:
    ...
    for internal_index, entry in df.iterrows():
        ...
        print("processing entry id", entry_id, judge_input)
        ...
        print("entry done, result", result_dict)
        # print(result[0])
    ...
    with open(os.path.abspath(f"../outputs/{agent_file_name}_results.json"), "w", encoding='utf-8') as fp:
